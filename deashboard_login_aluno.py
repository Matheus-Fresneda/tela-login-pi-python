...
def abrir_login_aluno(janela_pai=None):
    login_aluno = ctk.CTk()
    login_aluno.title("Login Aluno")
    login_aluno.geometry("1200x700")
    login_aluno.minsize(900, 600)
    login_aluno.configure(fg_color="#e6e6e6")
    login_aluno.state("zoomed")

    canvas = ctk.CTkCanvas(login_aluno, highlightthickness=0, bg="#e6e6e6")
    ...
    def desenhar_fundo(_event=None):
        ...
        largura = login_aluno.winfo_width()
        altura = login_aluno.winfo_height()
        canvas.delete("bg")
    ...
    login_aluno.bind("<Configure>", desenhar_fundo)
    desenhar_fundo()

    if logo_base is not None:
        logo_pil = logo_base.copy()
        logo_img = ctk.CTkImage(light_image=logo_pil, dark_image=logo_pil, size=(96, 38))
        logo_label = ctk.CTkLabel(login_aluno, text="", image=logo_img, fg_color="transparent")
    else:
        logo_label = ctk.CTkLabel(login_aluno, text="Etec", font=("Quicksand", 20, "bold"), text_color="#333333", fg_color="transparent")
    logo_label.place(x=20, y=14)

    # Frame central
    frame = ctk.CTkFrame(login_aluno, width=370, height=320, corner_radius=22, fg_color="#d1d1d1")
    ...
    entry_login = ctk.CTkEntry(frame, placeholder_text="xxxxxx@aluno.cps.sp.gov", width=270, height=38, font=("Quicksand", 15))
    ...
    def entrar():
        # Aqui você pode validar o login/senha e abrir o dashboard do aluno
